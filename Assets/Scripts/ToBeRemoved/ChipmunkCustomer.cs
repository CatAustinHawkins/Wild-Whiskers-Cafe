...
public class ChipmunkCustomer : MonoBehaviour
{
    ...
    public GameObject RoastedMushroom;
    ...
    public GameObject IbexCustomer;
    void Start()
    {
    ...
    void Update()
    {
        ...
        if (movingtofoh) //if the player is moving to the front of house
        {
            ...
            if (!MovingtoDesk && !IbexCustomer.activeSelf)
            {
    ...
    {
        RoastedMushroom.SetActive(true);
        StartCoroutine(Eating2());
        DialogueScript.ChipmunkMeal = false;
    }
    ...
    IEnumerator Eating2() //second eating coroutine
    {
        ...
        RoastedMushroom.SetActive(false);
        Coins.SetActive(true);
        PlateDrop.Play();
        Leaving = true; //make the customer leave
        DialogueScript.ChipmunkDone = true;
    }
}
