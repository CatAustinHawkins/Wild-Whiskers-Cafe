...
public class GiraffeCustomer : MonoBehaviour
{
    ...
    public GameObject LeafSalad; //REPLACE WITH FOOD
    ...
    void Update()
    {
        ...
        if (movingtofoh) //if the player is moving to the front of house
        {
            ...
            if (!MovingtoDesk && !PonyCustomer.activeSelf)
            {
    ...
    IEnumerator GiraffeLeaving() //waiting at the front of house desk - then they can move to the table
    {
    ...
    {
        LeafSalad.SetActive(true);
        StartCoroutine(Eating2());
        DialogueScript.GiraffeMeal = false;
    }
    ...
    IEnumerator Eating2() //second eating coroutine
    {
        ...
        DialogueScript.GiraffeDone = true;
    }
