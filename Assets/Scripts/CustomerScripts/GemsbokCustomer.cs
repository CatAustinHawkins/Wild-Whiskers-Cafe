...
public class GemsbokCustomer : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Leaving) //if the customer has been fed and is leaving
        {
            ...
            if (transform.position == LeavingArea.transform.position)
            {
                ...
                PangolinCustomer.SetActive(true);
            }
        }
    }
    ...
    public void FedSmoothie()
    {
        ...
        DialogueScript.GemsbokDrink = false;
    }

    public void FedMeal() //if the customer has been fed
    {
        PoachedMelon.SetActive(true);
        StartCoroutine(Eating2());
        DialogueScript.GemsbokMeal = false;
        DialogueScript.GemsbokDone = true;
        DialogueScript.IbexMeal = true;
    }
    ...
    IEnumerator Eating1()
    {
        ...
        DialogueScript.ThoughtIcons[18].SetActive(true); //the FOOD icon
        DialogueScript.ThinkBubble[11].SetActive(true); //thought bubble
        DialogueScript.GemsbokMeal = true;
    ...
    IEnumerator Eating2() //second eating coroutine
    {
        ...
        DirtyPlate.SetActive(true);
        PoachedMelon.SetActive(false);
        Coins.SetActive(true);
        PlateDrop.Play();
        Leaving = true; //make the customer leave
    }
    IEnumerator IbexSpawn()
    {
        yield return new WaitForSeconds(30f);
        IbexCustomer.SetActive(true);
    }
}
